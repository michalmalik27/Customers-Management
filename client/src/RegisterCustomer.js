...
const rules = {
  ...
  customerHebName: {
    pattern: {
      value: /^[\u0590-\u05fe-\s]+$/i,
      message: "Only hebrew characters '-' and space",
    },
  ...
  customerEngName: {
    pattern: {
      value: /^[A-Za-z-\s]+$/i,
      message: "Only english characters '-' and space",
    },
  ...
  const onSubmit = async (customer) => {
    try {
      ...
      if (response.ok) {
        ...
        setSaveStateText(
          `Save Failed: ${data.message || "Model Is Not Valid"}`
        );
      }
    } catch (e) {
      setSaveState(actionStates.IS_FAILED);
    ...
    <Container component="main">
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <SelectInput
                ...
                control={control}
                list={cities}
                extractId={(item) => item.cityId}
                extractValue={(item) => item.cityName}
                rules={rules.cityId}
              />
            </Grid>
            <Grid item xs={12} md={6}>
              <SelectInput
                ...
                control={control}
                list={banks}
                extractId={(item) => item.code}
                extractValue={(item) => `${item.description} (${item.code})`}
            ...
            <Grid item xs={12} md={6}>
              <SelectInput
                ...
                control={control}
                list={branches()}
                extractId={(item) => item.branchNumber}
                extractValue={(item) =>
                  `${item.branchName} (${item.branchNumber})`
                }
      ...
      <UIActionState actionState={saveState} actionText={saveStateText}>
        <div className={classes.alert}>
          <Alert severity="success">The customer saved!</Alert>
        </div>
      </UIActionState>
    </Container>
  );
